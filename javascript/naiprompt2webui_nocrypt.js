...
onUiUpdate(function () {
  let parentArea = gradioApp().querySelector("#txt2img_tools > .form");
  let parentArea2 = gradioApp().querySelector("#img2img_tools");
  let generateBtn = gradioApp().querySelector("#txt2img_generate");
  let beforeElement = gradioApp().querySelector("#roll_col");

  if (parentArea == null || generateBtn == null)
    return;
  if (gradioApp().querySelector("#nai2localconvert") != null) return;
  ...
  convertBtn.innerHTML = "NAI";
  convertBtn.title = "NAIConvert (NoCrypt)"
  convertBtn.className = "lg secondary gradio-button tool svelte-1ipelgc";
  convertBtn.addEventListener("click", onClickConvert);
  parentArea.appendChild(convertBtn);
  // parentArea2.insertBefore(convertBtn, parentArea2.children[0]);
